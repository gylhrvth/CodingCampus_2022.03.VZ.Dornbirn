...
public class HighlightHesse {
    ...
    public static void main(String[] args) {
        ...
        // Highlight user text
        highlightUserText(input);

    }

    public static void hesseHighlight() {

        String text = "Hesse";
        System.out.println(textToAnalyse.replaceAll(text, ANSI_BLACK + BACK_RED + text.toUpperCase() + ANSI_RESET));

    }

    public static String readUserText(Scanner scan) {

        return scan.nextLine();
    }

    public static void highlightUserText(Scanner scan) {

        System.out.println("Please enter your text to highlight.");
        String userText = readUserText(scan);

        String regEx = "";
        for (int i = 0; i < userText.length(); i++) {
            String singleChar = userText.substring(i, i + 1);
            regEx += ("[" + singleChar.toUpperCase() + singleChar.toLowerCase() + "]");
        }
        System.out.println(regEx);

        if (!textToAnalyse.matches(regEx)) {
            System.out.println("There is no such a text");
        } else {
            System.out.println(textToAnalyse.replaceAll(regEx, ANSI_BLACK + BACK_RED + userText.toUpperCase() + ANSI_RESET));
        }

        /*String hallo = "Hallo wie gehts?";
        String group = "[ag0-9]";
        String group2 = "(a|[0-9]|b)";
        String beginning = "^a";
        String end = "a$";
        String count = "a{2,4}";
        String minimumOne = "a+";
        String zeroToInfinity = "a*";
        String regex = "l{2,4}";
        String test = "[A-z]";
        System.out.println(hallo.replaceAll(test, "b"));*/
