...
public class SearchHesse {
    ...
    public static void indexOfHess(String text) {
        String textToSearch = "Hesse";
        int count = 0;
        /*
        int index = text.indexOf(textToSearch);
        while (index >= 0){
            count++;
            System.out.println("found Hesse at Index: " + index);
            index = text.indexOf(textToSearch, 1 + index);
        }
         */

        for (int i = 0; i < text.length(); i++) {
            if (text.startsWith("Hesse", i)) {
                System.out.println("found Hesse at Index:" + i);
                count++;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        String textToAnalyse = "Zur Zeit des Zweiten Weltkriegs waren seine großen Werke Siddhartha und Der Steppenwolf noch verboten. Heute gehört Hermann Hesse zu den bekanntesten deutschen Schriftstellern. Mehr über den Weltveränderer lest ihr hier\n" +
                "Hermann Hesse\n" +
                "\n" +
        ...
        // int hesseCount =  textToAnalyse.split("Hesse").length;
        // int hesse = textToAnalyse.indexOf("Hesse", 0);
        // System.out.println("The word Hesse shows up "+hesseCount+" times");
        // System.out.println(hesse);
        //indexOfHess(textToAnalyse);
        Scanner sc = new Scanner(System.in);
        String test = textToAnalyse;
        System.out.println("Please enter a word to Replace with all Upper");
        String word = sc.nextLine();
        String testnew = test.replace(word, word.toUpperCase());
